...
public Product getProduct(int prcode) {
	connectDB();
	sql = "select * from products where prcode=?";
	Product p = null;
	try {
		pstmt = conn.prepareStatement(sql);
...
public boolean newProduct(Product product) {
	...
	sql = "insert into products(prname,price,manufacture) values(?,?,?)";
	try {
		pstmt = conn.prepareStatement(sql);
...
public boolean delProduct(int prcode) {
	connectDB();
	sql = "delete from products where prcode=?";
	try {
		pstmt = conn.prepareStatement(sql);
		pstmt.setInt(1, prcode);
		pstmt.executeUpdate();
...
public boolean updateProduct(Product product) {
	connectDB();
	sql = "update products set prname=?, price=?, manufacture=? where prcode=?";
	try {
		pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, product.getPrname());
		pstmt.setInt(2, product.getPrice());
